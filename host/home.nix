    ...
    isDarwin = pkgs.stdenv.isDarwin; in
{
  ...
  home.activation.sync-vscode = lib.hm.dag.entryAfter [ "writeBoundary" "installPackages" "git" ] ''
    ...
    rm -f "$WINDOWS_VSCODE_SETTINGS" "$WINDOWS_VSCODE_KEYBINDINGS"

    # Copy config files
    cp "$HM_VSCODE_SETTINGS" "$WINDOWS_VSCODE_SETTINGS"
    cp "$HM_VSCODE_KEYBINDINGS" "$WINDOWS_VSCODE_KEYBINDINGS"
